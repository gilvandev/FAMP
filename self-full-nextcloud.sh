...
THIS SCRIPT WILL EXECUTE FOLLOWING THESE NEXT STEPS IN 15 SECONDS.
...
# 4.- HTTP headers will be installed and configured
# 5.- The TRACE method will be disabled
# 6.- Explicitely and exclusively allow the GET, POST and HEAD methods
# 7.- Install the Mod_Evasive module in Apache HTTP to help mitigate DoS attacks
# 8.- Install the Mod_Security module in Apache HTTP as a Web Application Firewall (WAF) setup with the default rules. Adjust to your needs.
'
...
# 3.1- Redirect HTTP connections to HTTPS (port 80 and 443 respectively)
# 3.1.1- Enabling the rewrite module
sed -i -e '/mod_rewrite.so/s/#LoadModule/LoadModule/' /usr/local/etc/apache24/httpd.conf

# 3.1.2- Adding the redirection rules from HTTP to HTTPS.
...
# 5.- Disable the TRACE method.
echo 'TraceEnable off' >> /usr/local/etc/apache24/httpd.conf

# 6.- Allow specific HTTP methods.
sed -i '' -e '272i\
...
# 7.4- Restart Apache for the configuration to take effect
apachectl graceful

# 8.- WAF-like rules enablement for any generic FAMP stack. Applicable to WPress, Drupal, Joomla, Nextcloud and the likes of those.
# Create an empty httpd-security.conf file
touch /usr/local/etc/apache24/extra/httpd-security.conf

# 8.1- Enabling the rewrite module
sed -i -e '/mod_rewrite.so/s/#LoadModule/LoadModule/' /usr/local/etc/apache24/httpd.conf
